...
export const FileUpload: React.FC<FileUploadProps> = ({ onFileChange, disabled, fileSelected }) => {
  const [fileName, setFileName] = useState<string>('Escolher Áudio no Dispositivo');
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      setFileName('Escolher Áudio no Dispositivo');
      onFileChange(null);
    }
  };
  ...
  return (
    <div className="w-full sm:w-auto">
      ...
      >
        {fileSelected ? <CheckIcon className="w-5 h-5 text-green-600 dark:text-green-500" /> : <UploadIcon className="w-5 h-5" />}
