...
import { Transcription, AudioRecording } from '../types';
...
const isTranscription = (item: any): item is Transcription => 'raw_transcript' in item;
...
export const PropertiesModal: React.FC<PropertiesModalProps> = ({ item, onClose, additionaData }) => {
  ...
  const renderContent = () => {
    if (isTranscription(item)) {
      const rawWordCount = (item.raw_transcript || '').split(/\s+/).filter(Boolean).length;
      const cleanedWordCount = (item.cleaned_transcript || '').replace(/<[^>]*>?/gm, ' ').split(/\s+/).filter(Boolean).length;
      return (
        <>
          <PropertyRow label="Ficheiro" value={item.filename} />
          <PropertyRow label="Data" value={new Date(item.created_at).toLocaleString('pt-PT')} />
          <PropertyRow label="ID de Áudio" value={item.audio_id || 'N/A'} />
