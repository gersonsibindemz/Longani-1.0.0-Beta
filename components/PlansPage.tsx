...
import { CheckIcon, StarIcon, UsersIcon, SparkleIcon } from './Icons';
    ...
    return (
        <main className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-grow">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                {plans.map((plan) => (
                    <div key={plan.id} className={`rounded-2xl p-8 border-2 flex flex-col ${profile.plan === plan.id ? 'border-[#24a9c5] shadow-2xl' : 'border-gray-200 dark:border-gray-700 bg-white/60 dark:bg-gray-800/60'}`}>
                        <div className="flex-grow">
                            <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-200">{plan.name}</h2>
                            <p className="mt-4 text-4xl font-bold tracking-tight text-gray-900 dark:text-white">{plan.price}</p>
